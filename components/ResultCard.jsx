...
const VERDICT_CONFIG = {
  LIFE_JACKET_CHECK_PASSED: {
    label: 'LIFE JACKET DETECTED',
    textColor: COLORS.green,
    bgColor: 'rgba(0, 230, 118, 0.10)',
    borderColor: 'rgba(0, 230, 118, 0.22)',
  },
  LIFE_JACKET_MISSING: {
    label: 'LIFE JACKET MISSING',
...
function DetectionBadge({ method }) {
  ...
  return (
    <View style={badge.pill}>
      <Text style={badge.text}>DEVICE AI</Text>
    </View>
  );
}
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.statsRow}>
        ...
        <View style={styles.statBox}>
          <Text style={styles.statLabel}>Wearing</Text>
          <Text style={[styles.statValue, { color: COLORS.green }]}>{result.wearing ?? 0}</Text>
        </View>
        <View style={styles.statDivider} />
        <View style={styles.statBox}>
          <Text style={styles.statLabel}>No Jacket</Text>
          <Text style={[styles.statValue, { color: COLORS.red }]}>{result.not_wearing ?? 0}</Text>
        </View>
      </View>

      {(result.uncertain ?? 0) > 0 && (
        <Text style={styles.uncertain}>{result.uncertain} detection(s) require manual confirmation.</Text>
      )}

      <MetricBar label="Confidence" value={confidence} color={confidenceColor} />
      <MetricBar
        label="Pass rate"
        value={result.compliance_rate ?? result.coverage ?? 0}
        color={COLORS.green}
      />
...
const styles = StyleSheet.create({
  ...
  statLabel: { fontSize: 9, fontWeight: '600', color: COLORS.muted, textTransform: 'uppercase', letterSpacing: 1.2, marginBottom: 3 },
  statValue: { fontSize: 18, fontWeight: '700' },
  uncertain: { color: COLORS.orange, fontSize: 11, marginTop: SIZES.xs },
